...
//    console.log(conf.groups);  //连续显示的分页数
    //if(conf.curr !== conf.pages && conf.next || dict.flow){
    if(conf.curr == conf.pages && conf.next || true){
        view.push((function(){
           // return (dict.flow && conf.curr === conf.pages)
        	return (conf.curr === conf.pages) 
            //? '<span class="page_nomore" title="\u5df2\u6ca1\u6709\u66f4\u591a">'+ conf.next +'</span>'
            ? '<span class="page_nomore" style="background-color: #f1eff0;cursor: pointer;">'+ conf.next +'</span>'
    ...
    //输出总页数
    view.push('<span class="pages">共&nbsp;'+conf.pages+'&nbsp;页</span>');
