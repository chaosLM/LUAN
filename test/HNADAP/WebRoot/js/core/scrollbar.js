...
function myScroll( el ) {    
	...
	var _myScroll = { 
		...
		renderDOM: function(){ 
			...
			$w.wrap('<div class="scroll-content"><div class="Scroller-Container scroll-content-inner"></div></div>');
			
			var $scrollContent = $w.parents('.scroll-content');
			$scrollContent.after('<div class="scroll-bar"><div class="Scrollbar-Track"><span class="Scrollbar-Handle"></span></div></div>');
		   	
		   	var $scrollBar = $scrollContent.siblings('.scroll-bar');  

		  	me._scrollObj.intro = {
		  		start: function() {
		  			if ($scrollContent.length) me._scrollObj.intro.iniScr();
		  	    },
		  	    iniScr: function(){
		  	    	var scroller  = null;
		  	        var scrollbar = null; 
		  	        scroller  = new Scrolling.Scroller($scrollContent[0], 430, 250);
		  	        scrollbar = new Scrolling.Scrollbar($scrollBar[0], scroller, new Scrolling.ScrollTween());
		  	    }
			}; 
		  	me.start();
		},
		start: function(){
			_myScroll._scrollObj.intro.start();  
		}
	};	
	
	_myScroll.wrapper = el;  
	_myScroll.init();
	return _myScroll;
};  
